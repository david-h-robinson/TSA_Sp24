...
for(i in 1:nhydro){
  par(mfrow=c(1,2))  #place plot side by side #par will work with acf or pacf
...
iHP <- 4 #choosing one column in our data frame and running all the analysis for that column #HP4
...
#There is a strong correlation between data set and time (low p-value) but only 26% (R squared) is explained by time.
...
beta0 <- linear_trend$coefficients[1] #selects the first of the coefficients
linear_trend$coefficients #check the two coefficients
beta1 <- linear_trend$coefficients[2]

y_detrend <- inflow_data[,iHP+1] - (beta0 + beta1*t) #beta0 and beta1 are the trend component #t is the independent variable
#then we subtract that trend from the original data set 
#the epsilon trend is part of the linear regression -- it remains in the series and is part of the random component
...
for(i in 1:nobs){
  seas_comp[i] <- beta0 + beta1 %*% dummies[i,] #this tells it to do element by element multiplication
} #in other words, this is matrix multiplication

ggplot(inflow_data,aes(x=my_date))+
  geom_line(aes(y=inflow_data[,(iHP+1)]), col="black")+
  geom_line(aes(y=seas_comp),col="orange")

#this did not produce the same output as shown in class
