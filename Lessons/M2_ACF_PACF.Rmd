...
ggplot(inflow_data,aes(x=my_date,y=HP1, group = 1))+ #added "group = 1"?
...
```{r}
ts_inflow_data <- ts(inflow_data[,2], start=c(1931,1),frequency=12) #frequency is how many times we repeat the data -- e.g., daily is 365, monthly is 12, yearly is 1 #the number after the year (in this case 1) specifies which month to start with -- so this starts with January of 1931
#specifying the frequency is crucial for identifying seasonal patterns
ts_inflow_data
...
autoplot(ts_inflow_data) + #autoplot is kind of a ggplot for time series objects
  xlab("Time") +
  ylab("Inflow") +
  labs(color="Reservoir")

#note that this is exactly what we got when we used the ggplot structure
...
HP1_acf <- acf(ts_inflow_data) #acf is the autocorrelation function #it will give us information about the time or the self-correlation of the time series -- how is one observation correlated with another of the same variable
#note that the output chart is periodic -- kind of sinusoidal -- there is some seasonality happening in the acf
#so january 12 months ahead (lag 12) is correlated ~0.6 with january 12 months prior... the correlation of december (lag 11) is less correlated with january 11 months prior...
...
HP1_pacf <- Pacf(ts_inflow_data,lag.max=40) #anything outside of the blue dotted lines can be considered "significant"
